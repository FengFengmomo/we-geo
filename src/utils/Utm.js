...
export class Utm {
    ...
    static latLng(options) {
        var ll = UC().UTM2LatLon(options);
        return ll;
    }

    static xy(lat, lon, zone, southHemi) {
        var dic = UC().LatLon2UTM(lat, lon, zone, southHemi);
        return dic;
    }
     // returns a new object normalized to the proper zone, band...
    static normalize(options){
        var tmp = this.latLng(options);
        return tmp ? tmp : null;
    }
}
...
function UC() {
    ...
    */
    function LatLonToUTMXY(lat, lon, zone, xy) {
        ...
        // 该地图的宽高分别为1000000米和20000000米，一百万米和一千万米
